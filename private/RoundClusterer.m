...
for i=1:length(matUnclustered) % mark the end of the first stance
    stIndices(i) = matUnclustered(i).StanceIndices(1,2);
end

roIndices = zeros(1,length(matUnclustered));
for i=1:length(matUnclustered) % mark the beginning of the round
...
YZPoints = zeros(length(matUnclustered),2); % mark the Y, Z position of the step
for i = 1:length(matUnclustered)
    YZPoints(i,:) = data(roIndices(i)+stIndices(i)-1,[4 5]);
end

clustResult = kmeans(YZPoints,2); % cluster in two groups based on the mean position
